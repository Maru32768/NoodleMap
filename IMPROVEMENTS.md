...
| 状態  | 項目              | 内容                                                            |
|-----|-----------------|---------------------------------------------------------------|
| 完了  | サブカテゴリ / タグ     | `tags`, `shops_tags` を追加                                      |
| 完了  | カテゴリ更新 TODO 解消  | 既存カテゴリ更新はタグ実装に合わせて再設計                                         |
| 未着手 | 開店日属性 / 新店判定    | `shops.opened_at` を追加し、開店から1年以内の店舗を新店として扱う                    |
| 未着手 | 開店日属性 / 未開店判定   | `shops.opened_at` を追加し、まだ未開店のお店が未開店だとわかるようにする                 |
| 未着手 | 閉店日属性 / 閉店判定    | `shops.closed_at`を追加し、もうすぐ閉店するお店がわかるようにし、閉店ステータスはこれを参照するようにする |
| 未着手 | 店舗リンク / SNS URL | `shop_links` を追加                                              |
| 未着手 | 訪問ログ            | `visit_logs` を追加し、複数回訪問やメニュー記録に対応                             |
| 未着手 | 店舗削除            | 関連データ削除または外部キー制約を整える                                          |
| 未着手 | 画像アップロード        | 実装する場合は `shop_images` と保存先を完成させる                              |
| 完了  | 店舗一覧取得の小バグ修正    | `FindRegisteredShops` の ID 配列生成を修正する                          |
...
| 未着手 | 検索/フィルタ拡張    | タグ絞り込みは対応済み。訪問ログ、リンク有無などで絞り込み   |
...
| 状態  | 項目         | 内容                                       |
|-----|------------|------------------------------------------|
| 未着手 | 店舗詳細の個別URL | `/search?shop=<id>` で選択店舗を開けるようにする |
...
- `server/shops/`: カテゴリ取得ロジックを定数判定に置き換え
- `web/src/features/categories/`: `useCategories` フックを削除
- フロントエンド各所: `allCategories` 引数を `getCategoryType` から除いて定数判定に変更
- `web/src/features/search/utils.ts`: `getCategoryType` をカテゴリ名ではなく `shop.categories` 内の固定スラッグで判定するよう変更
...
| 項目 | 詳細                                                                                                 |
|----|----------------------------------------------------------------------------------------------------|
| 場所 | `server/shops/handler.go` / `web/src/features/shops/shop-edit-modal.tsx:274` |
...
| 備考 | `shop_images` テーブルは存在し sqlc クエリも生成済みだが、API エンドポイントが未実装   |
...
|----|--------------------------------------------------|
| 場所 | フロント・バック共に未実装                                    |
| 状態 | `DELETE /shops/:id` エンドポイントなし、管理画面に削除ボタンなし |
| 影響 | 管理画面から店舗を消す手段がない                                 |

**提案**: `DELETE /shops/:id` エンドポイントと管理画面の削除ボタンを追加する。
...
| 問題                                                             | 場所                                                                                           | 深刻度 |
|----------------------------------------------------------------|----------------------------------------------------------------------------------------------|-----|
| 重複 `google_place_id` 登録時など、DB 制約エラーを user-safe な typed error に変換する処理がまだ不足している | `server/shops/` | 高 |
...
| `db/shops/query.sql`         | `ANY($1::uuid[])` → `IN (?, ?, ...)` に書き換え      | 小     |
...
- `auth/`, `shops/`, `categories/`: 機能単位の処理を置く。単純 CRUD は handler/repository だけでよい。
...
**実装状況:**
`tags` / `shops_tags` を追加済み。タグは `color` を必須にし、店舗一覧 API で店舗ごとのタグを返す。店舗追加/更新 API は
`tagIds` を受け取り、管理 UI 側でタグ選択を接続できる状態にした。
...
`category_id` は nullable にしておく。これにより「ラーメン配下の家系」のような分類タグと、「駅遠」「行列」のような横断タグを同じ仕組みで扱える。

### 14.1. 開店日属性 / 新店判定

新店は手動タグとして管理すると時間経過で古くなるため、タグではなく店舗属性から導出する。

**追加したい情報:**

- `shops.opened_at` に開店日を保存する
- 開店日から1年以内の店舗を「新店」として表示・検索できるようにする
- 開店日が不明な店舗は新店判定の対象外にする

**設計方針:**
`tags` に「新店」を追加せず、API またはフロントエンド側で `opened_at` から派生状態を計算する。これにより、時間が経った店舗から自動的に新店表示が外れる。
...
`shops` に `sns_url` を1本だけ追加するより、リンクテーブルとして複数持てるようにした方が後で困りにくい。URL のドメインから
...
- `/search?shop=<shop_id>`
- `/search?shop=<shop_id>&lat=35.xxxxxx&lng=139.xxxxxx`
- `/r/<shop_id>` 共有・OGP向けの短縮URL
...
- `selectedId` を search params の `shop` と同期する
- 初期表示時に `shop` param を読んで該当店舗を選択する
- 選択店舗が現在のフィルタ条件では非表示でも、ピンまたは詳細パネルは表示できるようにする
- 店舗詳細に共有ボタンを追加する
- `/r/:id` は通常ブラウザでは `/search?shop=:id` へ誘導し、SNSクローラー向けには OGP 付き HTML を返す
