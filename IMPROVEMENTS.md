...
**admin 権限の管理:**
`users.is_admin` フラグは残す。初回ログイン時に自分のメールアドレス (`ADMIN_EMAIL` 環境変数等) と照合して
`is_admin = true` をセットする方式が最もシンプル。
...
```
Set-Cookie: token=<jwt>; HttpOnly; Secure; SameSite=Strict; Path=/api
```

フロントは `Authorization: Bearer` ヘッダーを手動でセットする処理 (`request.ts:66-68`) が不要になり、
`credentials: "include"` を fetch に追加するだけになる。

> **Google OAuth と組み合わせる場合:** OAuth フローで発行した JWT を HttpOnly Cookie にセットするのが自然な流れ。両方まとめて対応するのが効率的。
...
| 項目  | 詳細                                                                 |
|-----|--------------------------------------------------------------------|
| 場所  | `server/auth/service.go:51-67`, `server/auth/service.go:126-131` |
...
| 項目 | 詳細                                                                                                 |
|----|----------------------------------------------------------------------------------------------------|
| 場所 | `server/restaurants/service.go:190` / `web/src/features/restaurants/restaurant-edit-modal.tsx:274` |
...
| 問題                                                              | 場所                                                    | 深刻度 |
|-----------------------------------------------------------------|-------------------------------------------------------|-----|
| `toastApiError()` がエラーオブジェクトを `JSON.stringify` してそのまま表示する       | `web/src/utils/toast.ts:11`                           | 高   |
| ログイン・登録フォームのバリデーションエラーが画面に表示されない (`required` 制約は効いているが UI に出ない) | `web/src/pages/login/index.tsx`, `register/index.tsx` | 中   |
...
| 問題                                                             | 場所                                                                                           | 深刻度 |
|----------------------------------------------------------------|----------------------------------------------------------------------------------------------|-----|
| `ctx.AbortWithError()` に生の Go エラーをそのまま渡しており、DBエラー文言がクライアントに漏れる | `server/auth/handler.go`, `server/restaurants/handler.go`, `server/categories/handler.go` 各所 | 高   |
| 重複メール・重複 `google_place_id` 登録時に DB 制約エラー文字列がそのままレスポンスに出る       | `server/auth/service.go:108`, `server/restaurants/`                                          | 高   |
| permission denied エラーが 403 ではなく 500 で返る                        | `server/restaurants/handler.go:40,61`                                                        | 中   |
| サーバー側でエラーがどこにもログ出力されていない                                       | 全ハンドラ                                                                                        | 中   |

**提案の方向性:**

- バックエンド: `AbortWithError` にはサニタイズ済みのメッセージを渡し、詳細は `log.Printf` でサーバーログに出す
- フロントエンド: `toastApiError()` をレスポンスボディの `error` フィールドを読んで表示するよう修正し、フォールバックに汎用メッセージを出す
- フォームは `<Field errorText={form.formState.errors.x?.message}>` を各入力欄に追加する
...
| 項目 | 詳細                                                    |
|----|-------------------------------------------------------|
| 場所 | `server/restaurants/service.go:67-69`                 |
| 状態 | `make([]uuid.UUID, len(rs))` の後に `append` しているため、先頭に空 UUID が `len(rs)` 件入る |
...
現在は `FindCategoriesByRestaurantIds` に余分な UUID が渡るだけで大きな障害にはなりにくいが、無駄な条件が増え、将来の不具合調査時にノイズになる。
...
| 未着手 | `local.env` の整理               | git 管理から外し、`local.env.example` に置き換える       |
...
| 状態  | 項目                    | 内容                                                |
|-----|-----------------------|---------------------------------------------------|
| 未着手 | TypeSpec導入            | TypeSpecによるスキーマ定義を導入し、ジェネレートしたコードをベースにAPIを作成･利用する |
| 未着手 | バックエンドのソフトウェアーキテクチャ再考 |                                                   |
| 未着手 | ID体系の再考               |                                                   |
...
| 状態  | 項目                     | 内容                             |
|-----|------------------------|--------------------------------|
| 未着手 | goose 導入               | `db/init.sh` 依存からマイグレーション管理へ移行 |
| 未着手 | PostgreSQL → SQLite 移行 | 個人利用・単一ホスト運用に寄せる               |
| 未着手 | バックアップ/リストア簡略化         | SQLite `.db` ファイルベースに変更        |
...
| 状態  | 項目                      | 内容                                            |
|-----|-------------------------|-----------------------------------------------|
| 未着手 | Google OAuth 移行         | メール/パスワード認証を削除                                |
| 未着手 | JWT を HttpOnly Cookie 化 | `localStorage` 保存をやめる                         |
| 未着手 | JWT の期限設定              | `exp` / `iat` を付与し、漏洩時の有効期間を短くする              |
| 未着手 | `temporary_users` 削除    | OAuth 移行に合わせて不要テーブルを削除                        |
| 未着手 | 登録/ログイン画面整理             | `POST /login`, `POST /register`, パスワードフォームを削除 |
| 未着手 | プロキシ信頼設定修正              | `SetTrustedProxies` を環境に合わせて制限                |
...
| 未着手 | 店舗一覧取得の小バグ修正   | `FindRegisteredRestaurants` の ID 配列生成を修正する |
...
| 未着手 | エラーハンドリング見直し | APIエラーを人間向けメッセージに整える   |
